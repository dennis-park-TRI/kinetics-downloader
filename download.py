...
def download_classes(classes, num_workers, failed_save_file, compress, verbose, skip, log_file):
  ...
  """
    ...
    for list_path, save_root in zip([config.TRAIN_METADATA_PATH, config.VAL_METADATA_PATH],
        ...
        pool = parallel.Pool(
            classes, data, save_root, num_workers, failed_save_file, compress, verbose, skip, log_file=log_file
        )
        pool.start_workers()
        pool.feed_videos()
        pool.stop_workers()
...
def download_test_set(num_workers, failed_log, compress, verbose, skip, log_file):
  ...
  """
    ...
    pool = parallel.Pool(
        None, data, config.TEST_ROOT, num_workers, failed_log, compress, verbose, skip, log_file=log_file
    )
    pool.start_workers()
    pool.feed_videos()
    pool.stop_workers()
...
def main(args):
    ...
    if args.all:
        ...
        for category in categories:
            download_category(
                category, args.num_workers, args.failed_log, args.compress, args.verbose, args.skip, args.log_file
            )
        ...
        if args.classes:
            ...
            download_classes(
                args.classes, args.num_workers, args.failed_log, args.compress, args.verbose, args.skip, args.log_file
            )

        if args.test:
            # download the test set
            download_test_set(
                args.num_workers, args.failed_log, args.compress, args.verbose, args.skip, args.failed_log
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--compress", default=False, action="store_true", help="compress videos using gzip (not recommended)"
    )
    parser.add_argument("-v", "--verbose", default=False, action="store_true", help="print additional info")
    parser.add_argument(
        "-s", "--skip", default=False, action="store_true", help="skip classes that already have folders"
    )
    parser.add_argument(
        "-l", "--log-file", help="log file for youtube-dl (the library used to download YouTube videos)"
    )
